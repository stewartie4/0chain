...
// unsafe
func (mndb *MemoryNodeDB) reachable(node, to Node) (ok bool) {
	...
	case *FullNode:
		for i := byte(0); i < 16; i++ {
			...
			if mndb.reachable(childNode, to) {
				return true
			}
		}
	}
...
func (mndb *MemoryNodeDB) ComputeRoot() (root Node) {
	...
	mndb.iterate(context.TODO(), func(ctx context.Context, key Key, node Node) (err error) {
		if root == nil {
			root = node
		} else if !IncludesNodeType(NodeTypeFullNode|NodeTypeExtensionNode, node.GetNodeType()) {
		} else if mndb.reachable(root, node) {
		} else if mndb.reachable(node, root) {
			root = node
		}
		return
	})
...
func (mndb *MemoryNodeDB) Validate(root Node) (err error) {
	...
	return mndb.iterate(context.TODO(), func(ctx context.Context, key Key, node Node) (err error) {
		if _, ok := nodes[StrKey(node.GetHashBytes())]; !ok {
			...
			err = common.NewError("nodes_outside_tree", "not all nodes are from the root")
		}
		return
	})
}
