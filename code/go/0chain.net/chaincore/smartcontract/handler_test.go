...
import (
	...
	mocks "0chain.net/mocks/chaincore/chain/state"
...
func TestExecuteWithStats(t *testing.T) {
	...
	b := mocks.StateContextI{}
	b.On("GetTrieNode", mock.AnythingOfType("string")).Return(
		func(_ datastore.Key) util.Serializable {
			return &util.SecureSerializableValue{Buffer: blob}
		},
		func(_ datastore.Key) error {
	...
	b.On("GetClientBalance", mock.AnythingOfType("string")).Return(
	...
	b.On("AddTransfer", mock.AnythingOfType("*state.Transfer")).Return(
		func(_ *state.Transfer) error {
			return nil
		},
	)
	b.On("InsertTrieNode", mock.AnythingOfType("string"), mock.AnythingOfType("*faucetsc.GlobalNode")).Return(
