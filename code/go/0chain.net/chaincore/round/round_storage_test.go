...
func TestRoundStartingStore(t *testing.T) {
	...
	t.Run("Get", func(t *testing.T) {
		...
		if err := storage.Put(entity0, 0); err != nil {
			t.Fatal(err)
		}
		if err := storage.Put(entity3, 151); err != nil {
			t.Fatal(err)
		}
		if err := storage.Put(entity1, 5); err != nil {
			t.Fatal(err)
		}
		if err := storage.Put(entity4, 251); err != nil {
			t.Fatal(err)
		}
		if err := storage.Put(entity2, 51); err != nil {
			t.Fatal(err)
		}
