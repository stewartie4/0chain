...
type Block struct {
	...
	*MagicBlock           `json:"magic_block,omitempty"`
	...
	SmartContextStates *statesc.SmartContractState `json:"sc_state"`
}
...
func (b *Block) CreateSmartContractState(prevBlock *Block) {
	...
	if len(hashes) != 0 && len(states) == 0 {
		var prevState *statesc.SmartContractState
		if prevBlock != nil {
			prevState = prevBlock.GetSmartContractState()
		}
		b.SmartContextStates.CreateFromHash(prevState, version)
		return
