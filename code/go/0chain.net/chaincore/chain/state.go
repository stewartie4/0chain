...
func (c *Chain) computeState(ctx context.Context, b *block.Block) error {
	...
	for _, txn := range b.Txns {
		...
		if err := c.UpdateState(b, txn); err != nil {
			b.SetStateStatus(block.StateFailed)
			Logger.Error("compute state - update state failed", zap.Int64("round", b.Round), zap.String("block", b.Hash), zap.String("client_state", util.ToHex(b.ClientStateHash)), zap.String("prev_block", b.PrevHash), zap.String("prev_client_state", util.ToHex(pb.ClientStateHash)), zap.Any("error", err))
