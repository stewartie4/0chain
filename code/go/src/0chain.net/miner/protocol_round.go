...
func (mc *Chain) StartNextRound(ctx context.Context, r *Round) *Round {
	lastRound := viper.GetInt64("development.last_round")
	if lastRound > 0 && r.Number == lastRound {
		return nil
	}
...
func (mc *Chain) HandleRoundTimeout(ctx context.Context) {
	...
	Logger.Info("before round restart", zap.Any("state", r.GetState()), zap.Any("round", r.Number), zap.Any("timeout_count", mc.GetRoundTimeoutCount()), zap.Any("proposed_blocks", len(r.GetProposedBlocks())), zap.Any("notarized_blocks", len(r.GetNotarizedBlocks())))
	r.Restart()
	Logger.Info("after round restart", zap.Any("state", r.GetState()), zap.Any("round", r.Number), zap.Any("timeout_count", mc.GetRoundTimeoutCount()), zap.Any("proposed_blocks", len(r.GetProposedBlocks())), zap.Any("notarized_blocks", len(r.GetNotarizedBlocks())))
	if r.vrfShare != nil {
